...
用户进一步明确用途是“先检索并筛选论文，再解释论文研究内容、模型与改进，通过阅读逐步认识可用的建模方法”。本次修改保留两个技能的职责，不将预先确定完整路线作为检索前置条件。

检索端保留人工知网操作方式，并明确查询建议与实际题录的区别。阅读端增加默认速读与按需精读，将“原方法或基线、具体改动、动机、效果证据”作为明确的分析项目；区分已有方法应用与方法改进。交接信息保留选文理由、阅读问题和材料层级，阅读端据此优先展开。
